...
public:
	const int FRAME_WIDTH = 60;//玩家宽度
	const int FRAME_HEIGHT = 60;//玩家高度
private:
	const int SPEED = 3;
...
public:
	Enemy() {
		...
		anim_left = new Animation (atlas_enemy_left, 60);
		anim_right = new Animation (atlas_enemy_right,60);
	...
	bool CheckBulletCollision(const Bullet& bullet) {
		...
		bool is_overlap_y = bullet.position.x >= position.y && bullet.position.y <= position.y + FRAME_HEIGHT;
		return is_overlap_x && is_overlap_y;
	}

	bool CheckPlayerCollision(const Player& player) {
		//判断矩形相交，转换为敌人中心点与角色矩形相交问题
		POINT check_position = { position.x + FRAME_WIDTH / 2,position.y + FRAME_HEIGHT / 2 };
		bool is_overlap_x = check_position.x >= player.GetPosition().x && check_position.x <=  player.GetPosition().y + player.FRAME_WIDTH;
...
void UpdateBullets(std::vector<Bullet>& bullet_list, const Player& player) {
	...
	double radiant_interval = 2 * 3.14159 / bullet_list.size();//子弹之间的弧度间隔
	POINT player_position = player.GetPosition();
	double radius = 100 + 25 * sin(GetTickCount() * RADIAL_SPEED);
	for (size_t i = 0; i < bullet_list.size(); i++)
	{
		double radian = GetTickCount() * TANGENT_SPEED + radiant_interval * i;//当前子弹所在弧度制
...
int main()
{
	...
	loadimage(&img_menu, _T("C:/Users/Administrator/source/repos/幸存者/幸存者/img/menu.png"));
